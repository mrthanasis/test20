...
def lastdaysresults(x):
    x=int(x)
    query=("""
    select a.UserType,a.deviceCategory, sum(a.transactions) as Conversions, sum(a.visits) as Sessions,
    ...
    lastxdays=df.groupby(['lastdays','date']).sum().eval("Conversion_rate=Conversions/Sessions")\
    ...
    .eval("Conversion_rate=Conversions/Sessions")[['Conversion_rate']]
    
    breakdown=df.assign(Period=np.where(df['lastdays']<=x,'ThisPeriod','PastPeriod')).groupby(['Period','UserType','deviceCategory'])\
    .sum().eval("Conversion_rate=Conversions/Sessions")[['Conversion_rate']].reset_index()
